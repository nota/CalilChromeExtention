...
CalilRender.prototype =  {
	...
	init_abstract : function(isbn){
		$("#"+isbn).html('<div class="calil_searching" style="display:inline-block;">検索中</div>');
	},
	render_abstract : function (isbn,systemid,data, conti){
		...
		text += '<a href="'+link+'" class="calil_status '+style.css+'" style="color:white">'+ style.status + '</a></div>';
	...
	//		log("before: " + before_s + " and after: " + status + ":" + isup);
			if (isup){
				if (data.reserveurl != ""){
					text += '<div>';
					text += '<a href="'+data.reserveurl+'"><img border="0" src="http://gyazo.com/2064f557b8c17c879558165b0020ff5e.png"></a>';
					text += '</div>';
				}
				$("#"+isbn).html(text);
...
'<div id="calil_place_dialog" style="text-align:left;">',
...
'		<h3 style="font:150% bold;margin: 10px 0;">図書館のエリアを選んでください。</h3>',
